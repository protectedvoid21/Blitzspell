...
public class Projectile : MonoBehaviour
{
    ...
    [SerializeField]
    private float raycastLookAheadDistance = 0.2f; // Dodatkowy dystans dla raycasta, aby wykryć obiekty tuż przed sobą
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, direction, out var hit, raycastDistance, hitLayer, QueryTriggerInteraction.Collide))
        {
            // Trafiliśmy w coś!
            Debug.Log(
    ...
    {
        Debug.Log($"Projectile trafił w: {hit.collider.name} na warstwie: {LayerMask.LayerToName(hit.collider.gameObject.layer)}");
        ...
        else
        {
            ...
            Debug.Log($"Hit object {hit.collider.name} is not Damageable and not a Shield. It's likely environment.");
